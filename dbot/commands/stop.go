...
func stopHandler(b *dbot.Bot) handler.CommandHandler {
	return func(e *events.ApplicationCommandInteractionCreate) error {
		...
		if err := player.Destroy(); err != nil {
			...
			err = e.CreateMessage(discord.MessageCreate{Content: "modules.music.commands.stop.error"})
			if err != nil {
		...
		if err := b.Client.Disconnect(context.TODO(), *e.GuildID()); err != nil {
			...
			err = e.CreateMessage(discord.MessageCreate{Content: "modules.music.commands.stop.disconnect.error"})
			if err != nil {
				b.Logger.Error("Failed to send message: ", err)
			}
			return err
		...
		return e.CreateMessage(discord.MessageCreate{Content: "modules.music.commands.stop.stopped"})
	}
}
