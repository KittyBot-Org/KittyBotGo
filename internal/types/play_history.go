...
func (b *Bot) AddPlayHistory(userID snowflake.Snowflake, title string, query string) {
	...
	// TODO: fix this lol
	if _, err := b.DB.NewDelete().Model((*models.PlayHistory)(nil)).Where("last_used_at <= (?)",
		b.DB.NewSelect().Column("last_used_at").Table("play_histories").Where("user_id = ?", userID).Order("last_used_at DESC").Limit(1).Offset(4),
